...
SHELL = /bin/sh
LDFLAGS = "-s -w -X github.com/tarampampam/mikrotik-hosts-parser/version.version=$(shell git rev-parse HEAD)"
...
build: ## Build app binary file
	docker-compose run $(DC_RUN_ARGS) -e "CGO_ENABLED=0" --no-deps app go build -trimpath -ldflags $(LDFLAGS) -o ./mikrotik-hosts-parser .
