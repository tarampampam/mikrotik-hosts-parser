...
func NewHandler(ctx context.Context, log *zap.Logger, cacher cache.Cacher, cfg *config.Config) (http.Handler, error) {
	...
	if ip := net.ParseIP(cfg.RouterScript.Redirect.Address); ip != nil {
		...
		h.defaultRedirectIP = net.IPv4(127, 0, 0, 1) // config contains wrong value
	}
...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.writeComment(w,
		...
		fmt.Sprintf("Cache lifetime: %s", h.cacher.TTL()),
	...
	for i := 0; i < len(params.sources); i++ {
		go func(ch chan<- hostsFileData, url string) {
			...
			// TODO without current goroutine execution - 119 allocs/op (totally)

			if hit, data, ttl, err := h.cacher.Get(url); hit && err == nil {
				if records, parsingErr := hostsfile.Parse(bytes.NewReader(data)); parsingErr == nil {
			...
			if records, err := hostsfile.Parse(data); err == nil {
				...
				ch <- hostsFileData{url: url, err: errors.New("cache writing error")}
			}
	...
	for i := 0; i < len(params.sources); i++ {
		...
		if data.cacheHit {
			h.writeComment(w, fmt.Sprintf("Cache HIT for <%s> (expires after %s)", data.url, data.cacheTTL))
		} else {
...
func (h *handler) writeComment(w http.ResponseWriter, comments ...string) {
...
func (h *handler) fetchRemoteSource(url string) (*bytes.Buffer, error) {
	...
	if cl := resp.Header.Get("Content-Length"); cl != "" {
		...
		if max := int(h.cfg.RouterScript.MaxSourceSizeBytes); value >= max {
			return nil, fmt.Errorf("Content-Length header value [%d] is too big (max: %d)", value, max)
		}
...
func (p *reqParams) fromValues(v url.Values) error {
	...
	if hosts, ok := v["excluded_hosts"]; ok {
